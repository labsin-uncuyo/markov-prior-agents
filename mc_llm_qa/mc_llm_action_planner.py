"""
@file mc_llm_action_planner.py

@brief Implementation of an LLM-based action planner for reactive agent systems,
       guided by a Markov Chain. Includes full tracing of MC and prompt logic.
...
planning actions based on observations, memory, and a Markov Chain that probabilistically determines
the next action type. Most of a the code is adapted from the original implementation in 
...
class MCLLMActionPlanner:
    def __init__(self, model_name: str, goal: str, memory_len: int = 10, api_url=None, config: dict = None, markov_chain_path: str = None):
        ...
        # Masked action templates: single [MASK] per field
        # We let the LLM fill only the parameter values while we fix the action name.
        self.masked_action_templates = {
            "ScanNetwork": {
                "action": "ScanNetwork",
                "parameters": {
                    "target_network": "[MASK]",
    ...
    def get_action_from_obs_react(self, observation: Observation, memory_buf: list) -> tuple:
        ...
        # ----------- STAGE 1: REASONING & PRIORITIZATION -----------
        ...
        # ----------- STAGE 2: PARAMETER FILLING (Masked JSON) -----------
        self.logger.info(f"(Stage 2) Filling parameters for action type: '{next_action_type_str}'...")

        # Normalize to canonical naming expected by validator
        canonical_action_name = (
            "FindServices" if next_action_type_str in ("FindServices", "ScanServices") else next_action_type_str
        )

        # Build masked JSON object with a single [MASK] per field
        masked_template = self.masked_action_templates.get(canonical_action_name)
        if masked_template is None:
            self.logger.error(f"Unknown action type for masked template: {canonical_action_name}")
            return False, {"action": "UnknownAction", "parameters": {"error": f"No template for {canonical_action_name}"}}, None, total_tokens_used

        masked_json_str = json.dumps(masked_template)

        # Instruction for constrained fill-in
        # Use Q2 from YAML to drive mask filling, appending the masked JSON template.
        q2_template = self.config['questions'][2]['text']
        q2_fill_prompt = f"{q2_template}\nAction (fill masks): {masked_json_str}"

        # Add the LLM's reasoning before asking it to fill the parameters
        messages.append({"role": "assistant", "content": reasoning_response})
        messages.append({"role": "user", "content": q2_fill_prompt})

        self.logger.info(f"Stage 2 (Masked) Full Prompt:\n{json.dumps(messages, indent=2)}")
        # Trace: store Stage 2 prompt
        self.prompts.append({"stage": "stage2", "messages": messages})

        final_json_response, usage2 = self.openai_query(messages, max_tokens=512, fmt={"type": "json_object"})
